...
export class Init1710817315561 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`expenses\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`amount\` int NOT NULL, \`date\` datetime NOT NULL, \`category_id\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`
    );
    await queryRunner.query(
    ...
    await queryRunner.query(`INSERT INTO \`categories\` (\`id\`, \`name\`) VALUES (1, 'Streaming')`);
    await queryRunner.query(`INSERT INTO \`categories\` (\`id\`, \`name\`) VALUES (2, 'Gaming')`);
    await queryRunner.query(`INSERT INTO \`categories\` (\`id\`, \`name\`) VALUES (3, 'Internet')`);
  }
